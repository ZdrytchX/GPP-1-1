...
void Cmd_CallVote_f( gentity_t *ent )
{
  ...
  gamemode = 0;
    ...
    {
       trap_SendServerCommand( ent-g_entities, va(
         "print \"g_mode_cpm: You cannot call for a game mode change after %d seconds\n\"",
         g_mapvoteMaxTime.integer ) );
       return;
    }
    else
    {
      trap_SendServerCommand( ent - g_entities, va( "print \"g_mode_cpm: Sorry, this voting feature is currently under construction.\n\"") );
 ...
 */
 void Cmd_Donate_f( gentity_t *ent ) {
  ...
  if( g_floodMinTime.integer )
   ...
   trap_SendServerCommand( ent-g_entities,
     va( "print \"Donated %d %s to your team mates.\n\"",
     total-value, type ) );
 }
